...
3. You can setup your own env variables for PORT and mongodb connection string, by creating a env file in /backend and defining below variables
   ...
   ```
   By default PORT is 3030 and DB_STRING is mongodb://localhost/hotelontouchdb

4. Startup Mongodb local server
   ```sh
   mongod
   ```
5. Now to startup the nodejs serer run
   ```sh
   node app.js
   ```
The Server is now running on [localhost](http:/localhost:3030)
<!-- USAGE EXAMPLES -->
## Usage
